...
const utils = require('./utils')
  ...
  , SceneLightState = require('node-hue-api').v3.model.lightStates.SceneLightState
  , LightStateShim = require('./LightStateShim')
  // New API
  , newApi = require('node-hue-api').v3.api
  , discovery = require('node-hue-api').v3.discovery
...
function HueApi(config) {
  ...
  const bootstrap = newApi.createLocal(config.hostname, config.port);

  self._initializing = bootstrap.connect(config.username, config.timeout)//TODO set no deprecation warnings in the API as they are not under user control here
    .then(api => {
      self._api = api;
...
HueApi.prototype.registerUser = function (deviceDescription, cb) {
  ...
  const promise = this._getNewApi().then(api => {
    return api.users.createUser('node_hue_api', deviceName, false);
  });
...
HueApi.prototype.lights = function (cb) {
  function generateResponseData(data) {
    ...
    data.forEach(light => {
      result.push(Object.assign({id: light.id}, light._bridgeData));
    });
...
HueApi.prototype.luminaires = function (cb) {
  const promise = this._getNewApi().then(api => {
    return api.groups.getLuminaries();
  });
...
HueApi.prototype.updateGroup = function (id, name, lightIds, cb) {
  ...
  parameters.forEach(function (param) {
    if (param instanceof Function) {
      ...
      payload.lights = param.map(value => `${value}`);
  ...
  const promise = this._getNewApi()
    .then(api => {
      return api.groups.updateAttributes(id, payload);
    });
